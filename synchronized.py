...
def recordatorio(intentMessage, e):
    ...
    print(content)
    mqttClient.publish_start_session_action(site_id=intentMessage, session_init_text = u'¿Te la has tomado?', session_init_intent_filter = ["ManuJazz:Affirmation", "ManuJazz:Negation"], session_init_can_be_enqueued=True, session_init_send_intent_not_recognized=True, custom_data=None)
...
def appointment_reminder(intentMessage, appointment):
    print("Evento detectado para: %s" % datetime.now())
    say(intentMessage, "Hola. Te recuerdo que manana a esta hora tienes una cita con el "+appointment.subject+" en "+appointment.place)
    content = "["+str(datetime.today())+"] Appointment announced: "+appointment.subject+" ("+appointment.place+")\n"
    print(content)
    with open("/var/lib/snips/skills/ManuJazz.ACHOSintex/monitoring_output.txt", "a") as text_file:
		...
		#checks constantly
			...
			if os.path.isfile(filename):
					...
					print(datastore)
						...
						print(prev)
						scheduler.add_job(appointment_reminder, 'date', run_date=prev, id=date, args=['default', a], max_instances=10000)
						logging.basicConfig()
					...
					for prescription in datastore['prescriptions']:
						#2. Prescription
							...
							identity = fecha+" - "+med
                               				scheduler.add_job(recordatorio, 'interval', days=1, start_date=date, id=identity, args=['default',e], max_instances=10000)
                                			logging.basicConfig()
				...
				with open("/var/lib/snips/skills/ManuJazz.ACHOSintex/monitoring_output.txt", "a") as text_file:
                        		...
                        		text_file.write(content)
				say('default', u"Sincronización detectada. Ahora las prescripciones y los medicamentos se han actualizado.")
				with open("/var/lib/snips/skills/ManuJazz.ACHOSintex/monitoring_output.txt", "a") as text_file:
...
def subscribe_taken_medicine(hermes, intentMessage):
	...
	taken_medicine(hermes, intentMessage, conf)


def taken_medicine(hermes, intentMessage, conf):
	mqttClient.publish_start_session_notification('default', 'Perfecto. Me lo apunto', None)
	mqttClient.publish_end_session('default', "")


def subscribe_not_taken_medicine(hermes, intentMessage):
        conf = read_configuration_file(CONFIG_INI)
        not_taken_medicine(hermes, intentMessage, conf)


def not_taken_medicine(hermes, intentMessage, conf):
        mqttClient.publish_start_session_notification('default', u'De acuerdo. Te lo recordaré dentro de un momento', None)
        mqttClient.publish_end_session('default', "")
